    ...
    <Box component="section" id="skills" className="section">
      ...
      >
        <div
          ...
          style={{ width: 400, height: 400, top: "-10%", left: "-10%", opacity: 0.36 }}
        />
      </Box>
            ...
            <FadeIn key={group.label} delay={gi * 80} y={22}>
              <Box sx={{ textAlign: "center" }}>
                ...
                >
                  <span
                    className="script"
                    style={{ fontSize: "0.85rem", color: group.accent }}
                  >
                ...
                >
                  {group.skills.map((skill, si) => (
                    <span
                      key={skill}
                      className={`tag ${
                        group.accent === "var(--terracotta)"
                          ? "tag--terracotta"
                          : group.accent === "var(--sage)"
                          ? "tag--sage"
                          : "tag--gold"
                      }`}
                      style={{ "--pop-delay": `${si * 60}ms` }}
                      data-cursor
                    >
                      {skill}
                    </span>
